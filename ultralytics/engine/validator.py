    ...
    def __call__(self, trainer=None, model=None):
        ...
        if self.training:
            ...
            for d in self.args.data:
                if str(d).split(".")[-1] in {"yaml", "yml"}:
                    self.data.append(check_det_dataset(d))
                else:
            ...
            model.eval()
            model.warmup(imgsz=(1 if pt else self.args.batch, self.args.input_channel, imgsz, imgsz))  # warmup
        ...
        for i in range(3, len(self.dataloader)):
            self.dataset = i
            ...
            bar = TQDM(self.dataloader[self.dataset], desc=self.get_desc(), total=len(self.dataloader[self.dataset]))
            ...
            self.speed = dict(
                zip(self.speed.keys(), (x.t / len(self.dataloader[self.dataset].dataset) * 1e3 for x in dt)))
